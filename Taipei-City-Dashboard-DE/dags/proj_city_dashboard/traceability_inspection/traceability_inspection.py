...
def _transfer(**kwargs):
    ...
    df["sampling_location"] = df.get("SamplingLocation", pd.Series(dtype=str))
    df["inspect_result"] = df.get("InspectResult", pd.Series(dtype=str))
    df["raw_data"] = df.apply(
        lambda row: json.dumps(row.to_dict(), ensure_ascii=False, default=str),
        axis=1,
    )
    df["data_time"] = now_str
    ready_data = df[["sampling_location", "inspect_result", "raw_data", "data_time"]]
...
dag = CommonDag(proj_folder="proj_city_dashboard", dag_folder="traceability_inspection")
